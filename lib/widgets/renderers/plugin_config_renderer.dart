...
class PluginConfigRendererView extends Renderer {
  PluginConfigRendererView({required pageController, required ViewContextController viewContext})
...
class _PluginConfigRendererViewState extends RendererViewState {
  ...
  Future<void> saveConfigValue() async {
    await plugin.setPropertyValue("config", PropertyDatabaseValueString(jsonEncode(configData)));
  }
  ...
  Widget get configWidget => Column(
        ...
        children: space(
            2,
            configJsonList.map((configJson) => getConfigWidget(configJson)).toList()
              ..add(Row(
                children: [
                  TextButton(
                    ...
                    onPressed: () async {
                      ...
                      await CVUActionNavigateBack().execute(pageController, CVUContext());
                    },
                    child: Text("Save"),
                  ),
                  SizedBox(width: 10),
              ...
              return ConstrainedBox(
                ...
                child: Container(
                  ...
                  child: MemriTextField.sync(
                      binding: Binding<String>(() => configData[configJson.name],
                          (newValue) => setConfigValue(configJson.name, newValue))),
                ),
              );
          }
        }()
