...
class _CVUTimelineItemState extends State<CVUTimelineItem> {
  ...
  Future init() async {
    icon = MemriIcon.getByName(
        widget.nodeResolver.propertyResolver.string("icon") ?? "arrow_right");
    title = widget.nodeResolver.propertyResolver.string("title") ?? "-";
    subtitle = widget.nodeResolver.propertyResolver.string("text");
  }
