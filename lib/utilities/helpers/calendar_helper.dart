...
class CalendarHelper {
  ...
  DateTimeRange wholeDay(DateTime date) {
    return DateTimeRange(
        start: startOfDay(date).dateTime, end: endOfDay(date).dateTime);
  }
  ...
  DateTime dateByUnits(DateTime date, List<Units> units) {
    ...
    return DateTime(
        date.year,
        units.contains(Units.MONTH) ? date.month : 1,
        units.contains(Units.DAY) ? date.day : 1,
        units.contains(Units.HOUR) ? date.hour : 0);
  }
}
