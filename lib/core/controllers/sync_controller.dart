  ...
  if (kIsWeb) {
    ...
    if (config is IsolateSyncConfig) {
      ...
      dbController.databasePool =
          Database.connect(await config.isolate.connect());
    }
  }
...
class SyncController {
  ...
  uploadFiles() async {
    var fileItemRecordToUpload =
        await ItemRecord.fileItemRecordToUpload(databaseController);
  ...
  downloadFiles() async {
    var fileItemRecordToDownload =
        await ItemRecord.fileItemRecordToDownload(databaseController);
    ...
    await bulkAction(
        ...
        completion: ((error) async {
          await databaseController.databasePool.transaction(() async {
            await ItemRecord.didSyncItems(
                syncPayload, error, databaseController);
            if (error != null) {
              lastError = error;
              await setState(SyncControllerState.failed);
              return;
    ...
    await bulkAction(
        ...
        completion: ((error) async {
          ...
          await databaseController.databasePool.transaction(() async {
            await ItemRecord.didSyncItems(
                syncPayload, error, databaseController);
            await ItemEdgeRecord.didSyncEdges(
                syncPayload, error, databaseController);
          });
    ...
    if (lastDateServerModifiedTimestamp == null) {
      await searchAction(
          ...
          completion: (data, error) async {
            ...
            lastDateServerModifiedTimestamp = responseObjects.isNotEmpty
                ? responseObjects.first["dateServerModified"] + 1
                : 0;
          });
    }
    var limit = 5000;
    var lastItem =
        await ItemRecord.lastSyncedItem(databaseController.databasePool);
    var dateServerModifiedTimestamp =
        lastItem != null && lastItem.dateServerModified != null
  ...
  Future<PodPayloadBulkAction> makeSyncSchemaPropertiesData() async {
    var schemaPropertyItems = (await databaseController.databasePool
            .itemRecordsCustomSelect("type = ? AND syncState = ?", [
      Variable("ItemPropertySchema"),
      Variable(SyncState.create.inString)
    ]))
        .map((item) => ItemRecord.fromItem(item));
    var syncPropertyItems = (await Future.wait(schemaPropertyItems
            .map((e) async => await e.schemaPropertyDict(databaseController))))
  ...
  Future<PodPayloadBulkAction> makeSyncSchemaEdgesData() async {
    var schemaEdgeItems = (await databaseController.databasePool
            .itemRecordsCustomSelect("type = ? AND syncState = ?", [
      Variable("ItemEdgeSchema"),
      Variable(SyncState.create.inString)
    ]))
        .map((item) => ItemRecord.fromItem(item));
    var syncEdgeItems = (await Future.wait(schemaEdgeItems
            .map((e) async => await e.schemaEdgeDict(databaseController))))
        .whereType<Map<String, dynamic>>()
        .toList();
  ...
  updateSyncPayloadWithEdges(PodPayloadBulkAction syncPayload) async {
    ...
    List<ItemRecord> allItemRecords = (await databaseController.databasePool
            .itemRecordsCustomSelect(
                "id IN ('${allItemRecordIDs.join("', '")}')", []))
        .map((item) => ItemRecord.fromItem(item))
        .toList();
    ...
    if (targetItemRecords.isNotEmpty) {
      await Future.forEach<ItemRecord>(targetItemRecords, (itemRecord) async {
        ...
        if (itemRecord.type == "Edge" &&
            itemRecord.syncState == SyncState.create) {
    ...
    var edges = await databaseController.databasePool.edgeRecordsCustomSelect(
        ...
        [Variable(SyncState.create.inString)]);
    syncPayload.createEdges = (await Future.wait(edges.map((edge) async =>
            await (ItemEdgeRecord.fromEdge(edge))
                .syncDict(databaseController))))
        .compactMap((edge) => edge?["_name"][0] ==
                "~" //TODO handling reverse edges, but why do we even need them?
            ? null
            : edge);
  }
  ...
  Future<List<ItemRecord>> getTargetItems(List<ItemRecord> parentItemRecords,
    ...
    List<ItemEdgeRecord> targetEdges =
        (await Future.wait(parentItemRecords.map((itemRecord) async {
    ...
    List<ItemRecord> addItems = (await databaseController.databasePool
            .itemRecordsCustomSelect(
                "row_id IN (${targetItemIDs.join(", ")}) AND (syncState = ? OR syncState = ?)",
                [
          Variable(SyncState.create.inString),
          Variable(SyncState.update.inString)
        ]))
        .map((item) => ItemRecord.fromItem(item))
        .toList();
  ...
  searchAction(
    ...
    if (completion != null)
      await completion(Utf8Decoder().convert(networkCall.bodyBytes), error);
  }

  uploadFile(String uuid,
      Future<void> Function(String? data, String? error)? completion) async {
    ...
    var request = await PodUploadRequest.uploadFile(
        fileURL: fileURL, connectionConfig: connectionConfig);
  ...
  downloadFile(String sha256, String fileName,
      Future<void> Function(String? error)? completion) async {
    var connectionConfig =
        currentConnection ?? await AppController.shared.podConnectionConfig;
