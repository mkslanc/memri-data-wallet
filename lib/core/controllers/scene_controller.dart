...
class SceneController extends ChangeNotifier {
  ...
  init([List<Map<String, dynamic>>? pages]) async {
    try {
      ...
      if (pages == null) {
        try {
          savedNavStackList =
              await NavigationStack.fetchAll(appController.databaseController);
        } catch (e) {
          //cleaning navigationState, if there are stale state
          await appController.databaseController.databasePool
              .navigationStateClear();
        }
  ...
  removePageController(memri.PageController pageController) {
    ...
    pageControllers
        .forEach((element) => element.topMostContext?.config.cols = null);
  ...
  reset({isFactoryReset = false}) {
    ...
    pageControllers.forEach((pageController) =>
        pageController.reset(isFactoryReset: isFactoryReset));
    pageControllers = [];
      ...
      return Future.wait<NavigationElement?>(
        ...
        switch ((await item.propertyValue("itemType"))?.asString()) {
          ...
          default:
            title = (await item.propertyValue("title"))?.asString();
            var targetViewName =
                (await item.propertyValue("sessionName"))?.asString();
  ...
  StreamController<List<ItemRecord>> navigationItemRecords =
  ...
  navigateToNewContext(
    ...
    CVUDefinitionContent viewDefinition = defaultDefinition ??
        appController.cvuController.viewDefinitionFor(
    ...
    pageController.isInEditMode.value =
        !viewArguments.args["readOnly"]!.value.value;
    ...
    var queryConfig = await DatabaseQueryConfig.queryConfigWith(
        ...
        inheritQuery: inheritDatasource
            ? pageController.topMostContext?.config.query
            : null,
    ...
    if (pageIndex < pageControllers.length - 1) {
      ...
      if (clearSecondaryVal is CVUValueConstant &&
          clearSecondaryVal.value is CVUConstantBool) {
        clearSecondary = clearSecondaryVal.value.asBool()!;
      }

      if (clearSecondary) {
        for (var index = pageIndex + 1;
            index < pageControllers.length;
            index++) {
          var secondaryPageController = pageControllers[index];
          secondaryPageController.topMostContext = null; // TODO: ??
          secondaryPageController.navigationController
              .setViewControllers(Empty());
          secondaryPageController.navigationStack.state = [];
          secondaryPageController.navigationStack =
              secondaryPageController.navigationStack;
        }
      }
    }
    ...
    pageController.navigationController.setViewControllers(SceneContentView(
        pageController: pageController, viewContext: newViewContextController));
  }

  memri.PageController? pageControllerByLabel(String label) {
    return pageControllers
        .firstWhereOrNull((pageController) => pageController.label == label);
  }

  exitEditMode() {
    pageControllers
        .forEach((pageController) => pageController.isInEditMode.value = false);
  }
