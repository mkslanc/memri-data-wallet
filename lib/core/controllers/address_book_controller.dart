...
class AddressBookController {
  ...
  static upsertContact(Contact contact) async {
    ...
    var identifier = (contact.identifier != null)
        ? "AddressBook-" + contact.identifier!
        : "AddressBook-NaN";
    ...
    if (existingContact == null) {
      ...
      await person.setPropertyValue(
          "addressBookId", PropertyDatabaseValueString(identifier),
          db: db);
    ...
    for (var phoneNumber in phoneNumbers) {
      try {
        Phone.PhoneNumber formattedPhone =
            await Phone.PhoneNumberUtil().parse(phoneNumber.value!);
    ...
    for (var address in contact.postalAddresses) {
      await saveAddress(
          address: address, person: existingContact, db: db.databasePool);
    }

    if (contact.givenName != null) {
      await existingContact.setPropertyValue(
          "firstName", PropertyDatabaseValueString(contact.givenName!),
          db: db);
    }
    if (contact.familyName != null) {
      await existingContact.setPropertyValue(
          "lastName", PropertyDatabaseValueString(contact.familyName!),
          db: db);
    }
    if (contact.dates.isNotEmpty) {
      for (var date in contact.dates) {
        if (date.label == "birthday" &&
          ...
          await existingContact.setPropertyValue(
              "birthDate",
              PropertyDatabaseValueDatetime(DateTime(
    ...
    if (contact.hasAvatar) {
      await saveAvatar(
          avatar: contact.avatar!, person: existingContact, dbController: db);
    }
  }

  static Future<ItemRecord?> phoneNumberItemForPhoneNumber(
      {required String phoneNumber,
      required int phoneRowId,
  ...
  static Future<bool> hasSameAddress(
    ...
    if (addresses.isNotEmpty) {
      for (var addressItem in addresses) {
        var foundAddress = await getAddressItem(
            address: address, addressRowId: addressItem.rowId!, db: db);
  ...
  static savePhoneNumber(
    ...
    if (person.rowId != null) {
      if (await hasSamePhoneNumber(
              phoneNumber: formattedPhoneNumber, personItem: person, db: db) ==
          false) {
        ...
        await ItemEdgeRecord(
                sourceRowID: person.rowId,
                name: "hasPhoneNumber",
                targetRowID: newPhoneItem.rowId)
            .save(db);
      }
    }
  }
  ...
  static saveAddress(
      ...
      required Database db}) async {
    if (person.rowId != null) {
      if (await hasSameAddress(address: address, personItem: person, db: db) ==
          false) {
        var newAddressItem = ItemRecord(type: "Address");
        await newAddressItem.save(db);
        if (address.city != null) {
          await newAddressItem.setPropertyValue(
              "city", PropertyDatabaseValueString(address.city!));
        }
        if (address.region != null) {
          await newAddressItem.setPropertyValue(
        ...
        await ItemEdgeRecord(
                sourceRowID: person.rowId,
                name: "address",
                targetRowID: newAddressItem.rowId)
            .save(db);
      }
    }
  }
  ...
  static saveAvatar(
    ...
    if (person.rowId != null) {
      ...
      await ItemEdgeRecord(
              sourceRowID: person.rowId,
              name: "profilePicture",
              targetRowID: newImageItem.rowId)
      ...
      try {
        ...
        await newFileItem.setPropertyValue(
            "sha256", PropertyDatabaseValueString(sha256),
            db: dbController);
        await newFileItem.setPropertyValue(
      ...
      await ItemEdgeRecord(
              sourceRowID: newImageItem.rowId,
              name: "file",
              targetRowID: newFileItem.rowId)
          .save(dbController.databasePool);
    }
  }

  static Future<ItemRecord?> personWithAddressBookID(String addressBookId,
      [Database? db]) async {
    db ??= AppController.shared.databaseController.databasePool;
    var addressBookReference = await db.itemPropertyRecordsCustomSelect(
        "name = ? AND value = ?",
        [Variable("addressBookId"), Variable(addressBookId)]);
    if (addressBookReference.isNotEmpty) {
      var person =
          await ItemRecord.fetchWithRowID(addressBookReference[0].item);
      if (person != null) {
        return person;
      }
    }
