...
class PageController extends ChangeNotifier {
  ...
  String label;
  MemriUINavigationController navigationController =
  ...
  init(String viewName,
    ...
    navStack ??=
        await NavigationStack.fetchOne(label, appController.databaseController);
    if (navStack != null) {
  ...
  navigateTo(index) {
    ...
    topMostContext =
        navStack.state.isNotEmpty ? makeContext(navStack.state.last) : null;
    navigationStack = navStack;
    ...
    var vc = SceneContentView(
        pageController: this,
        viewContext: topMostContext!); //TODO: is this right?
