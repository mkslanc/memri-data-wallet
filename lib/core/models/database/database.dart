import 'package:memri/core/models/database/date_time_converter.dart';
...
class Database extends _$Database {
  ...
  Future<Item?> itemRecordFetchWithUID(String uid) async {
    return await (select(items)..where((t) => t.id.equals(uid)))
        .getSingleOrNull();
  }

  Future<Item?> itemRecordFetchWithRowId(int id) async {
    return await (select(items)..where((t) => t.rowId.equals(id)))
        .getSingleOrNull();
  }

  Stream<Item?> itemRecordFetchWithRowIdStream(int id) {
    return (select(items)..where((t) => t.rowId.equals(id)))
        .watchSingleOrNull();
  }
  ...
  Future<Item?> itemRecordFetchOneByType(String type) async {
    return await ((select(items)..where((t) => t.type.equals(type)))..limit(1))
        .getSingleOrNull();
  }
  ...
  Future<int> itemRecordDelete(ItemRecord record) async {
    return await (delete(items)..where((tbl) => tbl.rowId.equals(record.rowId)))
        .go();
  }
  ...
  Future<List<Item>> itemRecordsCustomSelect(
    ...
    return await customSelect(
        ...
        readsFrom: {
          items,
          ...joinTables
        }).map((row) => Item.fromData(row.data, this)).get();
  }
    ...
    return transaction(() async {
      ...
      for (var property in properties) {
        ...
        if (itemType is String &&
          ...
          var recordRowId =
              await itemRecordInsert(ItemRecord(type: "ItemPropertySchema"));
          stringCompanions.addAll([
            StringsCompanion(
                item: Value(recordRowId),
                name: Value("itemType"),
      ...
      for (var edge in edges) {
        ...
        if (sourceType is String &&
          ...
          var recordRowId =
              await itemRecordInsert(ItemRecord(type: "ItemEdgeSchema"));
          stringCompanions.addAll([
            StringsCompanion(
                item: Value(recordRowId),
    ...
    await (delete(tbl)
          ..where((tbl) {
            if (tbl is Integers) {
              ...
              return tbl.item.equals(record.itemRowID) &
                  tbl.name.equals(record.name);
  ...
  Future<dynamic> itemPropertyRecordSave(ItemPropertyRecord record,
      {bool? isNew}) async {
    if (isNew == null) {
      var property = await itemPropertyRecordsCustomSelect(
          "name = ? AND item = ?",
          [Variable(record.name), Variable(record.itemRowID)]);
  ...
  Future<List<dynamic>> itemPropertyRecordsCustomSelect(String query,
    ...
    if (isFTS) {
      ...
      List<IntegerDb> intProps = await customSelect(
              "SELECT * from integers WHERE $query",
              variables: binding,
              readsFrom: {integers})
          .map((row) => IntegerDb.fromData(row.data, this))
          .get();
      List<StringDb> stringProps = await customSelect(
              "SELECT * from strings WHERE $query",
              variables: binding,
              readsFrom: {strings})
          .map((row) => StringDb.fromData(row.data, this))
          .get();
      List<RealDb> realProps = await customSelect(
              "SELECT * from reals WHERE $query",
              variables: binding,
              readsFrom: {reals})
          .map((row) => RealDb.fromData(row.data, this))
          .get();
  ...
  ItemPropertyRecordTableData getItemPropertyRecordTableData(
  ...
  Future<int> itemEdgeRecordDelete(ItemEdgeRecord record) async {
    await (delete(items)..where((tbl) => tbl.rowId.equals(record.selfRowID)))
        .go();
    return (delete(edges)..where((tbl) => tbl.self.equals(record.selfRowID)))
        .go();
  }
  ...
  Future<Edge?> edgeRecordSelect(Map<String, dynamic> properties) async {
    return await customSelect(
        ...
        readsFrom: {
          edges
        }).map((row) => Edge.fromData(row.data, this)).getSingleOrNull();
  }

  Future<List<Edge>> edgeRecordsSelect(Map<String, dynamic> properties,
      [int? limit]) async {
    return await customSelect(
  ...
  Future<List<Edge>> edgeRecordsCustomSelect(
      String condition, List<Variable<dynamic>> binding,
      {bool? deleted}) async {
    var query = "SELECT * from edges ";
    Set<ResultSetImplementation> readsFrom = {edges};
    if (deleted != null) {
      query +=
          " INNER JOIN items ON (edges.self = items.rowId AND items.deleted = ?) ";
  ...
  Future<List<Item>> edgeRecordsItemsCustomSelect(
    ...
    binding.addAll(
        properties.values.map((property) => Variable(property)).toList());
