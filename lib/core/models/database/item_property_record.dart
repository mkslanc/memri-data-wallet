...
class ItemPropertyRecord {
  ...
  static Future insertList(List<ItemPropertyRecord> records,
      {Database? db}) async {
    ...
    List<String> itemUIds = records
        .map((itemPropertyRecord) {
          ...
          if (itemPropertyRecord.itemRowID == null)
            currentUIds.add(itemPropertyRecord.itemUID!);
          return currentUIds;
        })
    ...
    records.forEach((itemPropertyRecord) {
      itemPropertyRecord.itemRowID ??=
          groupedItemRecords[itemPropertyRecord.itemUID!]!.rowId;
    });
