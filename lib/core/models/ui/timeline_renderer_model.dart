...
class TimelineRendererModel {
  ...
  init(
      ...
      required Future<DateTime?> Function(ItemRecord)
          itemDateTimeResolver}) async {
  ...
  static Future<List<TimelineGroup>> group(List<ItemRecord> data,
    ...
    List<MapEntry<DateTime, ItemRecord>> dataPairs =
        (await Future.wait(data.map((dataItem) async {
    ...
    var groupedByDetailLevel =
        Map<DateTime, List<MapEntry<DateTime, ItemRecord>>>();
  ...
  static List<TimelineElement> groupByType(
      List<MapEntry<DateTime, ItemRecord>> data,
      {required bool mostRecentFirst,
    ...
    groupedByType.forEach((type, itemsWithDate) {
      if (itemsWithDate.length > maxCount) {
        ...
        groups.addAll(itemsWithDate.mapIndexed((index, item) => TimelineElement(
            itemType: type,
            index: index,
            items: [item.value],
            date: item.key)));
