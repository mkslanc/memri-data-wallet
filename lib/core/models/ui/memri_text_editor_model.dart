...
class MemriTextEditorModel {
  ...
  MemriTextEditorModel({required this.title, body})
      : this.body = body ?? Future(() => "");
  ...
  static List<String?> splitHTML(String string) {
    try {
      ...
      var titleElement = doc.body?.children.firstWhere(
          (element) => element.localName == "h1" && element.id == "title");
  ...
  static Future<String> combineHTML(
      {Future<String?>? title, required Future<String> body}) async {
    try {
