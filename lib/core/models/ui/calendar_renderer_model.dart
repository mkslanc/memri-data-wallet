...
class CalendarCalculations {
  ...
  init(
    ...
    if (datesWithItems.isNotEmpty) {
      ...
      minDate =
          datesWithItemsKeys.map((el) => el.millisecondsSinceEpoch).reduce(min);
      maxDate =
          datesWithItemsKeys.map((el) => el.millisecondsSinceEpoch).reduce(max);
    } else {
      minDate = DateTime.now().millisecondsSinceEpoch;
      maxDate = DateTime.now().millisecondsSinceEpoch;
    }
    this.start = CalendarHelper()
        .startOfMonth(DateTime.fromMillisecondsSinceEpoch(minDate))
        .dateTime;
    this.end = CalendarHelper()
        .endOfMonth(DateTime.fromMillisecondsSinceEpoch(maxDate))
        .dateTime;
  }
